...
const GallerySection = ({ limit }: Props) => {
  ...
  // On home (limit set): show `limit` photos + link. On full page: paginate in-page.
  const visiblePhotos = limit ? photos.slice(0, limit) : photos.slice(0, visibleCount);
  const hasMoreLink = limit ? photos.length > limit : false;
  const hasLoadMore = !limit && visibleCount < photos.length;
  ...
  return (
    <section id="gallery" className="section-padding bg-card/30" ref={ref}>
            ...
            <>
              ...
              {hasLoadMore && (
                <div className="flex justify-center mt-8">
                  <button onClick={() => setVisibleCount((c) => c + 12)} className="px-6 py-2.5 text-xs font-display font-semibold text-primary border border-primary/40 rounded-lg hover:bg-primary/10 transition-colors">
                    Load More ({photos.length - visibleCount} left)
                  </button>
                </div>
              )}
              {hasMoreLink && (
                <ShowMoreLink to="/gallery" count={photos.length - (limit ?? 0)} label="See full gallery" />
              )}
            </>
          ) : (
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
      ...
      <AnimatePresence>
        {selectedPhoto && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={closeLightbox} className="fixed inset-0 z-50 bg-background/90 backdrop-blur-sm flex items-center justify-center p-4">
