...
const mergeWithDefaultProjects = (databaseProjects: typeof defaultProjects) => {
  const projectsByTitle = new Map(databaseProjects.map((p) => [p.title.toLowerCase(), p]));
  const missingDefaults = defaultProjects.filter((p) => !projectsByTitle.has(p.title.toLowerCase()));
...
const ProjectsSection = ({ limit }: Props) => {
  ...
  const safeActive = Math.min(activeProject, Math.max(visible.length - 1, 0));
  ...
  return (
    ...
    >
      ...
      <div className="relative max-w-6xl mx-auto">
        ...
        >
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-primary/10 border border-primary/30 mb-4 sm:mb-6">
        ...
        {hasMore && (
          <ShowMoreLink to="/projects" count={projects.length - (limit ?? 0)} label="See all projects" />
        )}
      </div>
    </section>
  );
